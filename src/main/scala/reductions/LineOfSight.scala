...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    @tailrec
    def findMax(index: Int, maximum: Float): Float = {
      if (index == until) maximum
      else findMax(index + 1, max(input(index) / index, maximum))
    }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    val length = end - from
    if (length > threshold) {
      val splitIndex = length / 2
      val (leftLeaf, rightLeaf) = parallel(
        upsweep(input, from, from + splitIndex, threshold),
        upsweep(input, from + splitIndex, end, threshold)
      )
